...
class ConfigTab(QWidget):
    ...
    def setup_simulation_parameters_group(self):
        ...
        self.thread_count_input.setValidator(
            QIntValidator(1, get_thread_count()))
        ...
        self.time_step_input.setValidator(
            QDoubleValidator(0.0, float('inf'), 9))
        ...
        self.temperature_input, self.temperature_units, self.temperature_converted = self.create_simulation_field(
            "Temperature:", QLineEdit, ["K", "F", "C"], "K", "0.0")
        ...
        self.pressure_input.setValidator(
            QDoubleValidator(0.0, MAX_PRESSURE, 9))
        ...
        self.solver_parameters["maxIterations"] = self.create_solver_params_field(
            additional_layout_left, "Max Iterations:", "1000")
        self.solver_parameters["convergenceTolerance"] = self.create_solver_params_field(
            additional_layout_left, "Convergence Tolerance:", "1e-20")
        self.solver_parameters["outputFrequency"] = self.create_solver_params_field(
            additional_layout_left, "Output Frequency:", "1")
        self.solver_parameters["numBlocks"] = self.create_solver_params_field(
            additional_layout_left, "Num Blocks:", "30")
        self.solver_parameters["blockSize"] = self.create_solver_params_field(
            additional_layout_left, "Block Size:", "1")

        self.solver_parameters["maxRestarts"] = self.create_solver_params_field(
            additional_layout_right, "Max Restarts:", "20")
        self.solver_parameters["flexibleGMRES"] = self.create_solver_params_field(
            additional_layout_right, "Flexible GMRES:", ["false", "true"], is_combobox=True)
        self.solver_parameters["orthogonalization"] = self.create_solver_params_field(
            additional_layout_right, "Orthogonalization:", ["ICGS", "IMGS"], is_combobox=True)
        self.solver_parameters["adaptiveBlockSize"] = self.create_solver_params_field(
            additional_layout_right, "Adaptive Block Size:", ["false", "true"], is_combobox=True)
        self.solver_parameters["convergenceTestFrequency"] = self.create_solver_params_field(
        ...
        self.solver_parameters["maxIterations"][0].setValidator(
            QIntValidator(1, 1000000))
        self.solver_parameters["convergenceTolerance"][0].setValidator(
            QRegExpValidator(exp_regexp))
        self.solver_parameters["outputFrequency"][0].setValidator(
            QIntValidator(1, 1000))
        self.solver_parameters["numBlocks"][0].setValidator(
            QIntValidator(1, 1000))
        self.solver_parameters["blockSize"][0].setValidator(
            QIntValidator(1, 1000))
        self.solver_parameters["maxRestarts"][0].setValidator(
            QIntValidator(0, 1000))

        # Adding tooltips to different fields
        self.solver_parameters["maxIterations"][0].setToolTip(
            "Max Iterations: The maximum number of iterations the solver will perform. Range: 1 to 1'000'000.")
        self.solver_parameters["convergenceTolerance"][0].setToolTip(
            "Convergence Tolerance: The tolerance for the relative residual norm used to determine convergence. Range: 1e-50 to 1e-1.")
        self.solver_parameters["outputFrequency"][0].setToolTip(
            "Output Frequency: Determines how often information is printed during the iterative process. Range: 1 to 1'000.")
        self.solver_parameters["numBlocks"][0].setToolTip(
            "Num Blocks: Sets the number of blocks in the Krylov basis, related to the restart mechanism of GMRES. Range: 1 to 1'000.")
        self.solver_parameters["blockSize"][0].setToolTip(
            "Block Size: Determines the block size for block methods. Range: 1 to 1'000.")
        self.solver_parameters["maxRestarts"][0].setToolTip(
            "Max Restarts: Specifies the maximum number of restarts allowed. Range: 0 to 1'000.")
        self.solver_parameters["flexibleGMRES"][0].setToolTip(
            "Flexible GMRES: Indicates whether to use the flexible version of GMRES. Options: true, false.")
        self.solver_parameters["orthogonalization"][0].setToolTip(
            "Orthogonalization: Specifies the orthogonalization method to use. Options: ICGS, IMGS.")
        self.solver_parameters["adaptiveBlockSize"][0].setToolTip(
            "Adaptive Block Size: Indicates whether to adapt the block size in block methods. Options: true, false.")
        self.solver_parameters["convergenceTestFrequency"][0].setToolTip(
            "Convergence Test Frequency: Specifies how often convergence is tested (in iterations). Default setting is used if negative. Range: -1 or positive integers.")
            ...
            for key, (input_field, units_combobox) in self.solver_parameters.items():
                if key in config:
                    if isinstance(input_field, QLineEdit):
                        ...
                        index = input_field.findText(
                            config[key], Qt.MatchFixedString)
                        if index >= 0:
        ...
        if self.mesh_file.endswith('.stp'):
            ...
            if dialog.exec() == QDialog.Accepted:
                ...
                try:
                    ...
                    self.convert_stp_to_msh(
                        self.mesh_file, mesh_size, mesh_dim)
